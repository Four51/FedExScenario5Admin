...
function BaseLeftController(ComponentList, CurrentUser) {
    var vm = this;
    vm.currentUser = CurrentUser;
...
function resourceaccess() {
    ...
    return function(resources, user) {
        var userLimits = {
            Supplier: [
                "Orders",
                "Order History"
        ...
        angular.forEach(resources, function(resource) {
            if (!user.xp || !user.xp.UserType) {
                result.push(resource);
            } else if (userLimits[user.xp.UserType].indexOf(resource.Display) > -1) {
                result.push(resource);
            } else {
                angular.noop();
            }
